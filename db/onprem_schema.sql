...
CREATE TABLE IF NOT EXISTS customer_identity_map (
    ...
    created_dt           TIMESTAMP    DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY uq_map (global_id, domain),
    INDEX idx_source (source_customer_id),
    CONSTRAINT fk_identity_customer FOREIGN KEY (global_id) REFERENCES master_customer(global_id)
);

CREATE TABLE IF NOT EXISTS customer_pii_secure (
    pii_token            VARCHAR(20)  NOT NULL PRIMARY KEY,
    global_id            VARCHAR(30)  NOT NULL,
    customer_name_enc    VARCHAR(255),
    rrn_enc              VARCHAR(255),
    mobile_enc           VARCHAR(255),
    email_enc            VARCHAR(255),
    address_enc          VARCHAR(500),
    created_dt           TIMESTAMP    DEFAULT CURRENT_TIMESTAMP,
    updated_dt           TIMESTAMP    DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    INDEX idx_global (global_id),
    CONSTRAINT fk_pii_customer FOREIGN KEY (global_id) REFERENCES master_customer(global_id)
);
...
CREATE TABLE IF NOT EXISTS customer_360_profile (
    ...
    lifesync_score       DECIMAL(5,2),
    last_calc_dt         TIMESTAMP    DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT fk_360_customer FOREIGN KEY (global_id) REFERENCES master_customer(global_id)
);
...
CREATE TABLE IF NOT EXISTS consent (
    ...
    created_dt           TIMESTAMP    DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY uq_consent (global_id, domain),
    CONSTRAINT fk_consent_customer FOREIGN KEY (global_id) REFERENCES master_customer(global_id)
);
...
CREATE TABLE IF NOT EXISTS token_map (
    ...
    field_name           VARCHAR(30)  NOT NULL DEFAULT 'global_id',
    original_hash        VARCHAR(64)  NOT NULL,
    global_id            VARCHAR(30)  NOT NULL,
    created_dt           TIMESTAMP    DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY uq_hash (original_hash),
    INDEX idx_global (global_id),
    CONSTRAINT fk_token_map_global_id FOREIGN KEY (global_id) REFERENCES master_customer(global_id)
);
