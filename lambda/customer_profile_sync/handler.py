...
역할:
  ...
  2. On-Prem users.global_id 업데이트 (변경 시에만)
  3. global_id 반환
"""
import json
import os
...
def _fetch_global_id(email, domain):
...
def handler(event, context):
    ...
    try:
        global_id = _fetch_global_id(email, domain)
    ...
    # 2. On-Prem users.global_id 업데이트 (이미 같은 값이면 스킵)
    db = _get_db()
    try:
